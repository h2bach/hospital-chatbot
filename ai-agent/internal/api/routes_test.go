...
func TestSessionCannotBeReadFromAnotherDevice(t *testing.T) {
	staticDir := t.TempDir()
	...
	sessionID, err := sessions.CreateForOwner("device-a")
	if err != nil {
		t.Fatal(err)
	}
	...
	request := httptest.NewRequest(http.MethodGet, "/c/"+sessionID, nil)
	request.Header.Set("X-Device-ID", "device-b")
	response := httptest.NewRecorder()
	server.httpServer.Handler.ServeHTTP(response, request)
	if response.Code != http.StatusBadRequest {
		t.Fatalf("cross-device GET status = %d, want %d", response.Code, http.StatusBadRequest)
	}
}

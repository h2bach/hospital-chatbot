...
func (a *Agent) callUnifiedDetailed(ctx context.Context, input string, agentContext *domain.Context, report ProgressReporter) (*Result, error) {
	...
	default:
		...
		evidence = a.executePlan(ctx, *plan, agentContext.Role)
		if len(evidence) < maxUnifiedToolCalls && looksReferentialFollowup(input) {
			evidence = a.retryReferentialKnowledge(ctx, input, *plan, agentContext.Role, evidence)
		}
		...
		if len(evidence) < maxUnifiedToolCalls {
			if expanded, ok := a.expandExactProcessContext(ctx, initialKnowledge, agentContext.Role); ok {
	...
	trimConversation(agentContext, 24)
...
func (a *Agent) executePlan(parent context.Context, plan executionPlan, role domain.AccessRole) []toolEvidence {
	...
	for _, tool := range toolsForRole(tools, role) {
...
func (a *Agent) expandExactProcessContext(ctx context.Context, knowledge []*knowledgeToolEnvelope, role domain.AccessRole) (toolEvidence, bool) {
...
func (a *Agent) retryReferentialKnowledge(ctx context.Context, original string, plan executionPlan, role domain.AccessRole, evidence []toolEvidence) []toolEvidence {
	for _, task := range plan.Tasks {
